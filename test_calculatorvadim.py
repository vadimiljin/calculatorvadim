...
def test_validate_init_method():
    ...
    assert str(error.value.args[0]) == "Please enter a valid number for the calculator's memory value!"

def test_complex_number():
    calc = calculatorvadim.Calculator(-9)
    with pytest.raises(ValueError) as error:
        calc.root(2)
    assert str(error.value.args[0]) == "The root of a complex or negative number is not allowed."